...
process.env['BLENO_DEVICE_NAME'] = 'TAMPI ' + device_id;
...
var tampiState = new TampiState();
var tampiService = new TampiService( tampiState );
var deviceInfoService = new DeviceInfoService( 'CWRU', 'TAMPI', device_id);
...
bleno.on('stateChange', function(state) {
  if (state === 'poweredOn') {
    ...
    //
    bleno.startAdvertising('TampiService', [tampiService.uuid, deviceInfoService.uuid], function(err)  {
      if (err) {
        console.log(err);
      }
    });
